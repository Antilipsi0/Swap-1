...
if ( ! function_exists( 'haru_fonts_url' ) ) {
    function haru_fonts_url() {
        ...
        $outfit = _x( 'on', 'Plus Jakarta Sans font: on or off', 'teespace' );
        $yellow = _x( 'on', 'Smooch font: on or off', 'teespace' );
...
if ( ! function_exists( 'haru_enqueue_styles' ) ) {
    function haru_enqueue_styles() {
        ...
        if ( file_exists( $style_dir . '/teespace/style-custom.min.css' ) && ! defined( 'HARU_DEVELOPE_MODE' ) ) {
            wp_enqueue_style( 'haru-theme-style', $style_uri . '/teespace/style-custom.min.css' );
        } else {
            /* Theme CSS */
