...
if ( class_exists( 'WooCommerce' ) ) {
    ...
    if ( ! function_exists( 'haru_woocommerce_wishlist' ) ) {
        function haru_woocommerce_wishlist(){
            ...
            <a href="<?php echo esc_url( $wishlist_page ); ?>" class="haru-wishlist" title="<?php echo esc_attr__( 'Wishlist', 'teespace' ); ?>">
    ...
    if ( ! function_exists( 'haru_wc_color_variation_attribute_options' ) ) :
        ...
        function haru_wc_color_variation_attribute_options( $args = array() ) {
            ...
            echo '</select>';
            echo '<ul class="list-inline variable-attribute-wrap color-attribute-wrap">';
            if ( ! empty( $options ) ) {
                if ( $product && taxonomy_exists( $attribute ) ) {
                    $terms = wc_get_product_terms( $product->get_id(), $attribute, array( 'fields' => 'all' ) );
    ...
    if ( ! function_exists( 'haru_wc_image_variation_attribute_options' ) ) :
        ...
        function haru_wc_image_variation_attribute_options( $args = array() ) {
            ...
            echo '<ul class="list-inline variable-attribute-wrap image-attribute-wrap">';
            if ( ! empty( $options ) ) {
                if ( $product && taxonomy_exists( $attribute ) ) {
                    $terms = wc_get_product_terms( $product->get_id(), $attribute, array( 'fields' => 'all' ) );
    ...
    if ( ! function_exists( 'haru_wc_label_variation_attribute_options' ) ) :
        ...
        function haru_wc_label_variation_attribute_options( $args = array() ) {
            ...
            echo '<ul class="list-inline variable-attribute-wrap label-attribute-wrap">';
            if ( ! empty( $options ) ) {
                if ( $product && taxonomy_exists( $attribute ) ) {
                    $terms = wc_get_product_terms( $product->get_id(), $attribute, array( 'fields' => 'all' ) );
    ...
    if ( ! function_exists('haru_woocommerce_template_single_size_guide') ) {
        function haru_woocommerce_template_single_size_guide() {
            ...
            if ( $haru_product_size_guide ) {
                echo '<div class="product-size-guide"><a href="'. esc_url( $haru_product_size_guide ) .'" data-fancybox="product-guide" class="product-size-guide-link">' . esc_html__( 'Product Guide', 'teespace' ) .'</a></div>';
            }
        }
    }
    ...
    if ( ! function_exists( 'haru_woocommerce_empty_cart_button' ) ) {
        function haru_woocommerce_empty_cart_button() {
            echo '<a href="' . esc_url( add_query_arg( 'empty_cart', 'yes' ) ) . '" class="button empty-cart-button" title="' . esc_attr( 'Clear Shopping Cart', 'teespace' ) . '">' . ( wp_is_mobile() ? esc_html( 'Clear Cart', 'teespace' ) : esc_html( 'Clear Shopping Cart', 'teespace' ) ) . '</a>';
        }
